...
const Delete = ({
  ...
  content = 'You"re going to delete salary deduction, are you sure ?',
  button = 'Yes, Delete',
  isOpenModal,
  setIsOpenModal,
  data,
  ...
  const handleSuccess = () => {
    window.alert('Salary Deduction Deleted Successfully');
    closeModal();
  };
  ...
  function deleteSalaryDeduction(e: React.FormEvent) {
    e.preventDefault();

    form.delete(
      route('salary_deduction.destroy', { salaryDeduction: data.id }),
      {
  ...
  return (
    <>
      <DialogModal isOpen={isOpenModal} onClose={closeModal}>
        <form onSubmit={deleteSalaryDeduction}>
