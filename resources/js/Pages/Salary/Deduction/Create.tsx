...
const Create = ({
  ...
  content = 'For recording salary deduction for employee',
  button = 'Save',
  isOpenModal,
  setIsOpenModal,
  parentId,
  ...
  function createSalaryDeduction(e: React.FormEvent) {
    e.preventDefault();
    const { processing, ...formData } = form; // exclude 'error' and 'processing' properties

    console.log(form.data);
    form.post(route('salary_deduction.store'), {
  ...
  return (
    <>
      <Alert on={isCreateSuccess} setOn={setIsCreateSuccess}>
        Salary Deduction Created Successfully
      </Alert>

      <DialogModal isOpen={isOpenModal} onClose={closeModal}>
        <form onSubmit={createSalaryDeduction}>
